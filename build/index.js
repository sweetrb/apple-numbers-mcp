...
server.registerTool("add-rows", {
    ...
    inputSchema: {
        ...
        rows: z
            ...
            .min(1)
            .max(MAX_BATCH)
            .describe("Rows to append (array of arrays, one per row). A null in any cell position is a no-op — that cell is left unchanged rather than cleared."),
        sheet: z.string().optional().describe("Sheet name (default: first sheet)"),
        table: z.string().optional().describe("Table name (default: first table)"),
    },
    outputSchema: {
    ...
    inputSchema: {
        ...
        updates: z
            ...
            values: z
                ...
                .describe("Column values for the entire row. A null in any position is a no-op — that cell is left unchanged rather than cleared."),
        }))
            .min(1)
            .max(MAX_BATCH)
