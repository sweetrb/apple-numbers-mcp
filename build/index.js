...
// health-check
server.tool("health-check", "Use when: you want a quick check that the Python 3 read sidecar (numbers-parser) is installed and reports its version.\n" +
    "Returns: ok/not-ok plus the numbers-parser version string.\n" +
    "Do not use when: you need a full setup diagnostic including Numbers.app and Automation permission — use doctor instead.", {}, withErrorHandling(() => {
...
// doctor
server.tool("doctor", "Use when: a tool returns a permission or setup error, or you want the full setup diagnostic before writing/formatting.\n" +
    ...
    "Do not use when: you only need the lightweight read-sidecar version check — use health-check instead.", {}, withErrorHandling(() => {
    const report = runDoctor(manager);
    return successResponse(formatDoctorReport(report), { ...report });
}, "doctor"));
// get-file-info
server.tool("get-file-info", "Use when: you need to discover the structure of a .numbers file before reading or writing — the exact sheet and table names, their dimensions, and header rows. Call this first when sheet/table names are unknown.\n" +
...
// read-table
server.tool("read-table", "Use when: you want the data from a table — headers plus rows, optionally limited to a row range (0-based inclusive indices; header is row 0, so data starts at row 1) and/or a subset of columns. Defaults to the first sheet and first table if sheet/table are omitted.\n" +
...
// search
server.tool("search", "Use when: you need to locate where a text value appears across every cell in a .numbers file (case-insensitive partial match), optionally limited to one sheet.\n" +
...
// export-table
server.tool("export-table", "Use when: you want to write a table's data out to a CSV, TSV, or JSON file on disk. Defaults to the first sheet and first table if sheet/table are omitted.\n" +
...
// get-cell
server.tool("get-cell", "Use when: you need a single cell's value by 0-based row and column index (header is row 0). Set verbose=true to also get the formula, formatted value, and merge info.\n" +
...
// create-spreadsheet
server.tool("create-spreadsheet", "Use when: you want to create a brand-new .numbers file with a single sheet and table from a list of headers, optionally with initial data rows.\n" +
...
// set-cell
server.tool("set-cell", "Use when: you need to write a computed value to one cell in an existing .numbers file, addressed by 0-based row and column (header is row 0). Defaults to the first sheet/table if omitted.\n" +
...
// set-cells-batch
server.tool("set-cells-batch", "Use when: you need to write computed values to multiple cells at once (each with 0-based row/col); more efficient than many set-cell calls. Defaults to the first sheet/table if omitted.\n" +
...
// add-rows
server.tool("add-rows", "Use when: you want to append new rows of data to the end of an existing table; rows are added after the last existing row. Defaults to the first sheet/table if omitted.\n" +
...
// delete-rows
server.tool("delete-rows", "Use when: you need to permanently remove a contiguous range of rows from a table; both startRow and endRow are 0-based inclusive indices (header is row 0). Defaults to the first sheet/table if omitted.\n" +
...
// add-sheet
server.tool("add-sheet", "Use when: you want to add a new sheet to an existing .numbers file, optionally naming its default table and providing its headers.\n" +
...
// add-table
server.tool("add-table", "Use when: you want to add a new table to an existing sheet, optionally naming it and providing its headers. Defaults to the first sheet if omitted.\n" +
...
// import-csv
server.tool("import-csv", "Use when: you want to convert an existing CSV, TSV, or JSON file into a new .numbers spreadsheet. Format is auto-detected from the input file extension unless you specify it explicitly.\n" +
...
// update-rows
server.tool("update-rows", "Use when: you need to replace whole rows by 0-based index in an existing file; each update carries a row index and a complete array of column values. Carrying multiple {row, values} entries is more efficient than per-row writes. Defaults to the first sheet/table if omitted.\n" +
...
// rename-sheet
server.tool("rename-sheet", "Use when: you want to change a sheet's name in a .numbers file. Defaults to the first sheet if the current name is omitted.\n" +
...
// rename-table
server.tool("rename-table", "Use when: you want to change a table's name in a .numbers file. Defaults to the first sheet/table if omitted.\n" +
...
// set-formula
server.tool("set-formula", 'Use when: you need to write a live formula (e.g. "=SUM(A2:A10)") into a cell so it computes in Numbers, addressed by 0-based row/col. Requires explicit sheet and table.\n' +
...
// set-formulas-batch
server.tool("set-formulas-batch", "Use when: you need to write live formulas to multiple cells at once (each with 0-based row/col); more efficient than many set-formula calls. Requires explicit sheet and table.\n" +
...
// set-cell-style
server.tool("set-cell-style", "Use when: you need to format one cell — font, text/background color, number format, alignment, text wrap — addressed by 0-based row/col. Requires explicit sheet and table.\n" +
...
server.tool("set-cells-style-batch", "Use when: you need to format multiple cells at once (each with 0-based row/col); more efficient than many set-cell-style calls. Requires explicit sheet and table.\n" +
...
// set-column-width
server.tool("set-column-width", "Use when: you need to set a column's width in pixels, addressed by 0-based column index. Requires explicit sheet and table.\n" +
...
// set-row-height
server.tool("set-row-height", "Use when: you need to set a row's height in pixels, addressed by 0-based row index. Requires explicit sheet and table.\n" +
...
// merge-cells
server.tool("merge-cells", "Use when: you need to merge a rectangular range of cells into one, given 0-based inclusive top-left and bottom-right coordinates. Requires explicit sheet and table.\n" +
...
// unmerge-cells
server.tool("unmerge-cells", "Use when: you need to split a previously merged range back into individual cells, given 0-based inclusive top-left and bottom-right coordinates. Requires explicit sheet and table.\n" +
