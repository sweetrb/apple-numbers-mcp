...
export function registerResourcesAndPrompts(server: McpServer, manager: NumbersManager): void {
  // --- Resources ---
  ...
  server.resource(
    "file-info",
    new ResourceTemplate("numbers://file/{path}", { list: undefined }),
    (uri, variables) => {
      try {
  ...
  server.resource(
    "table",
    new ResourceTemplate("numbers://table/{path}", { list: undefined }),
    (uri, variables) => {
      try {
        const path = decodeURIComponent(String(variables.path));
  ...
  server.prompt(
    ...
    { path: z.string().describe("Path to the .numbers file to analyze") },
    ({ path }) => ({
      messages: [
        {
          role: "user",
